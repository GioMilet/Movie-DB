...
class Person {
  ...
  private $birthDate;
    ...
    */
    ...
    {
        $this->wroteMovies = new ArrayCollection();
     ...
     */
    public function getWroteMovies(): Collection
    {
        return $this->wroteMovies;
    }

    public function addWroteMovie(Movie $wroteMovie): self
    {
        if (!$this->wroteMovies->contains($wroteMovie)) {
            $this->wroteMovies[] = $wroteMovie;
            $wroteMovie->addWriter($this);
        }

        return $this;
    }

    public function removeWroteMovie(Movie $wroteMovie): self
    {
        if ($this->wroteMovies->contains($wroteMovie)) {
            $this->wroteMovies->removeElement($wroteMovie);
            $wroteMovie->removeWriter($this);
        }
